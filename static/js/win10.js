...
window.Win10 = {
    ...
    _bind_open_windows:function () {
		...
		// 注册事件委派 打开url窗口
        $('#win10').on('click', '.win10-open-window-caidan', function() {
            //>> 获取当前点击的对象
            $this = $(this);
            //>> 判断url地址是否为空 如果为空 不予处理
            if($this.data('url') !== "") {
    ...
    aboutUs: function() {
        ...
        layer.open({
            ...
            '<p>简易的WIN10界面</p>' +
            '<p><a href="./admin/" target="_blank" >桌面后台</a>----' +
            '<p>欢迎各位提点意见!</p>' +
			'<p>站长QQ:232619123 <a href="http://TV.nxog.top" target="_blank" >TV.nxog.top</a></p>' +
            '</div>'
        });
    },
